...
class MLFCore:
    ...
    def log_conda_environment(reports_output_dir: str):
        ...
        conda_env_filehandler = open(f'{reports_output_dir}/test_mlf_conda_environment.yml', 'w')
        subprocess.call(['conda', 'env', 'export', '--name', 'test_mlf'], stdout=conda_env_filehandler)
        print('[bold blue]Uploading conda environment report as a run artifact...')
        mlflow.log_artifact(f'{reports_output_dir}/test_mlf_conda_environment.yml', artifact_path='reports')
